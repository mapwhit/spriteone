...
export function pack(imgs) {
  ...
  const layout = Object.fromEntries(items.map(item => [item.id, extract(item)]).sort(keyComparator));
...
export function grid(imgs, { dim, columns = 12 }) {
...
export function scale(spriteLayout, scale) {
  if (scale === 1) {
    return spriteLayout;
  }
  ...
  function mapValues(source, mapFn, thisArg) {
    return Object.fromEntries(Object.entries(source).map(([k, v], index) => [k, mapFn.call(thisArg, v, index)]));
  }
}
